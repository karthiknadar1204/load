...
def _embed_images_in_html(
    ...
    """
    Replace img src="path" with data URIs using in-memory image bytes.
    Used when exporting to bytes (no filesystem). Paths are keys in images (rel path).
    """
    def replace_src(match: re.Match) -> str:
        full = match.group(0)
        path = match.group(2).strip()
        if path in images:
            raw, mime = images[path]
            b64 = base64.b64encode(raw).decode("ascii")
            return full.replace(path, f"data:{mime};base64,{b64}", 1)
        return full

    return re.sub(r'src=(["\'])([^"\']+)\1', replace_src, html_content)


def build_html_document_bytes(
    html_lines: List[str],
    images: Dict[str, Tuple[bytes, str]],
) -> bytes:
    """
    Build full HTML document from lines and in-memory images, return as UTF-8 bytes.
    No filesystem access; for use with ParseResult.export(..., "html").
    """
    html_content = "\n".join(html_lines).strip() + "\n"
    html_content = re.sub(r"\n{3,}", "\n\n", html_content)
    html_content = _embed_images_in_html(html_content, images)
